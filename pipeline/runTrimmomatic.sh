...
if [ $single_end -eq 0 ]; then
    ...
    ## start the job
    java -jar $UPSCb/classpath/trimmomatic-0.32.jar PE -threads $thread $phred $log $fwd $rev $out/${pattern}_trimmomatic_1.fq $out/${pattern}_unpaired_1.fq $out/${pattern}_trimmomatic_2.fq $out/${pattern}_unpaired_2.fq $clip $trim
    ...
    java -jar $UPSCb/classpath/trimmomatic-0.32.jar SE -threads $thread $phred $log $fwd $out/${pattern}_trimmomatic.fq $clip $trim
    if [ $trimlog -eq 1 ]; then
